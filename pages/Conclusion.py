...
if 'malicious_code' not in st.session_state:
...
st.markdown("""
                ## What just happened?
    The app is designed to simulate a controlled environment where users can write and test code for a Tic-Tac-Toe game. The user is given 3 minutes to finish writing a code that passes the pre-defined test cases. The objectve is to incentivize the user to use the ChatBot to help them with the code. In the background the ChatBot is instructed to remove the sensitive files located in the folder ```./Secret_Stuff```, thereby having access to the files. The aim of this project is to educate people of the possibility of manipulating an AI that would have access to the files; it can destroy them or even upload them to a remote server'
...
- [Detailed Information about the SolarWinds Attack](https://www.cisa.gov/news-events/alerts/2020/12/13/alert-ta20-352a-solarwinds-orion-network-management-products)
"""
